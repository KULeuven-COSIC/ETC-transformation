...
#if NO_PRECOMPUTE_HASH_INIT || VECLEN==1
...
#if NO_PRECOMPUTE_HASH_INIT
static void cshake_left_encode(hash_ctx_t ctx, unsigned s) {
    unsigned n=0, ss=s;
    do { n++; ss>>=8; } while(ss);
...
static void cshake256_init(hash_ctx_t ctx, const char *N, const char *S){
    unsigned l = N ? strlen(N) : 0;
    memset(ctx,0,sizeof(hash_ctx_t));
    cshake_left_encode(ctx,CSHAKE256_RATE);
    cshake_left_encode(ctx,8*l);
    hash_update(ctx,(const unsigned char *)N,l);
    l = S ? strlen(S) : 0;
    cshake_left_encode(ctx,8*l);
    hash_update(ctx,(const unsigned char *)S,l);
    keccakf(ctx);
}
#endif
...
#if NO_PRECOMPUTE_HASH_INIT
    cshake256_init(ctx,NULL,"ThreeBears");
#else
