...
#ifndef THREEBEARS_KARATSUBA
#define THREEBEARS_KARATSUBA 2
#endif

#if THREEBEARS_KARATSUBA==1 || THREEBEARS_KARATSUBA==2
...
#if defined(__BMI2__) && !defined(THREEBEARS_NO_ASM) && (WORD_BITS==64)
...
#endif /* THREEBEARS_KARATSUBA==1 || THREEBEARS_KARATSUBA==2 */

/** Accumulate out = c + a */
void add(gf_t out, gf_t const c, gf_t const a) {
    dslimb_t accum = 0;
...
void mac(gf_t c, const gf_t a, const gf_t b) {
    ...
    c[2] += accum1;
#else
#error "THREEBEARS_KARATSUBA must be 1 or 2"
#endif
}
...
void expand(gf_t ll, const uint8_t ch[GF_BYTES]) {
    limb_t buffer=0,tmp;
    for (unsigned i=0,j=0,bbits=0;j<NLIMBS;i+=sizeof(limb_t)) {
        if (i+sizeof(limb_t) >= GF_BYTES) {
            unsigned target = GF_BYTES-sizeof(limb_t);
            tmp = read_limb(&ch[target]) >> 8*(i-target);
        } else {
            tmp = read_limb(&ch[i]);
        }
