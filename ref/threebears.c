...
void decapsulate(
    ...
    /* recalculate */
    uint8_t *lpr_data2 = &capsule2[GF_BYTES*DIM];
    /* recalculate r*/
    hash_ctx_t ctx2;
    threebears_hash_init(ctx2,HASH_PURPOSE_ENCAPS);
    hash_update(ctx2,pk,MATRIX_SEED_BYTES);
    hash_update(ctx2,seed,ENC_SEED_BYTES + IV_BYTES);
    
    /* recalculate s'*/
    gf_t sk_expanded[DIM],b2,c2;
    for (unsigned i=0; i<DIM; i++) {
        noise(sk_expanded[i],ctx2,i);
    }
    ...
    noise(c2,ctx2,2*DIM);
    for (unsigned i=0; i<DIM; i++) {
        expand(b2, &pk[MATRIX_SEED_BYTES+i*GF_BYTES]);
        // printf("bsp: %d; c2 %d ||", bsp[0], c2[0]);
        mac(bsp,b2,sk_expanded[i]);
        // mac(c2,b2,sk_expanded[i]);
        // printf("bsp: %d; c2 %d\n", bsp[0], c2[0]);
    }
    add(c2, bsp, c2);
    canon(c2);
    ...
    for (unsigned i=0; i<ENC_BITS; i+=2) {
        limb_t h = tbi[i/8] >> (i%8);
        unsigned rlimb0 = (c2[i/2]          >> (LGX-LPR_BITS)) + (h<<3);
        unsigned rlimb1 = (c2[DIGITS-i/2-1] >> (LGX-LPR_BITS)) + ((h>>1)<<3);
        lpr_data2[i/2] = (rlimb0 & 0xF) | rlimb1<<4;
    }
    ...
    for (unsigned i=0; i<DIM; i++) {
        noise(c2,ctx2,i+DIM);
        for (unsigned j=0; j<DIM; j++) {
            uniform(b2,pk,j+DIM*i);
            mac(c2,b2,sk_expanded[j]);
        }
        contract(&capsule2[i*GF_BYTES], c2);
    }
    ...
    hash_output(ctx2,shared_secret,SHARED_SECRET_BYTES);
